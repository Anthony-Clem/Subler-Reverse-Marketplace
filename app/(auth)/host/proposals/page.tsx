...
export default function HostProposalsPage() {
  ...
  const { data: proposals, isLoading: proposalsLoading, error: proposalsError } = useHostProposals();
    ...
    return (
      <div className="max-w-md mx-auto my-12 bg-white border border-neutral-200/60 rounded-2xl p-8 text-center shadow-xs">
        ...
        <p className="text-body-sm text-slate-500 leading-relaxed mb-6">
          You must apply and be approved as a host by an administrator to view sent proposals.
        </p>
        <Link
          href="/host/upgrade"
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-neutral-200/80">
        ...
        <Link
          href="/host/dashboard"
          className="inline-flex h-10 items-center justify-center px-4 rounded-lg bg-[#ffb13d] text-[#0e1442] text-body-sm font-bold hover:bg-[#ffb13d]/90 transition-all active:scale-[0.98] cursor-pointer shadow-xs"
        >
      ...
      {proposalsError && !proposalsLoading && (
        <div className="p-6 rounded-xl bg-red-500/5 border border-red-500/10 text-center">
          ...
          <p className="text-body text-red-600 font-semibold">Failed to load proposals</p>
      ...
      {!proposalsLoading && !proposalsError && (!proposals || proposals.length === 0) && (
        <div className="bg-white border border-neutral-200/60 rounded-2xl p-8 md:p-12 text-center shadow-xs">
          ...
          <h3 className="text-lg font-bold text-[#0e1442] mb-2">No proposals submitted yet</h3>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 pb-4 border-b border-neutral-100">
                  <div>
                    ...
                    {request ? (
                      ...
                      <h3 className="font-display text-lg font-bold text-[#0e1442] tracking-tight">Matched Request</h3>
                    )}

                    <p className="text-caption text-slate-400 mt-1">
                      Submitted to {renterEmail} • Sent on {getFormatDate(proposal.createdAt)}
                    </p>
                  </div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex h-9 items-center justify-center gap-1.5 px-4 rounded-lg bg-[#ffb13d] text-[#0e1442] hover:bg-[#ffb13d]/90 text-body-sm font-bold transition-all cursor-pointer self-start sm:self-auto shadow-xs active:scale-[0.98]"
                  >
                ...
                {request && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-body-sm pb-4 border-b border-border/50 bg-neutral-50/50 p-4 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[10px] text-muted-foreground font-semibold">Renter Budget</p>
                        <p className="font-semibold text-foreground text-caption">${request.budget}/hr max</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-foreground text-caption">{request.headcount} people</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-foreground text-caption truncate max-w-[130px]">{request.locationPreference}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-foreground text-caption truncate max-w-[130px]">{getFormatDate(request.startDate)}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  <h4 className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider">Your Pitch Message</h4>
